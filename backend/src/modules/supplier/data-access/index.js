...
async function getSupplierProducts(sequelize, supplierId, options = {}) {
  ...
  if (options && typeof options === 'object') {
    ...
    if (rawFilters.length > 0) {
      ...
      for (const f of rawFilters) {
        ...
        if (isUuid(f.value) || field.includes('category_id') || field.includes('fabric_type_id') || field.includes('supplier_id') || field.includes('id')) {
  ...
  const sql = `SELECT p.*, c."name" as "category_name", COUNT(*) OVER()::int as total_count
     FROM "products" p
